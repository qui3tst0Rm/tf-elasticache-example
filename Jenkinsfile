pipeline{
    ...
    stages {
        ...
        stage ('write-file') {
            steps {
                script {
                    ...
                    writeFile(file:'/var/lib/jenkins/env_vars/Elasticache_Config_Endpoint.txt', text: 'TBC')
                ...
                sudo chown jenkins:jenkins /var/lib/jenkins/env_vars/Elasticache_Config_Endpoint.txt
        ...
        stage("set-instance-ips") {
            steps {
                sh '''
                terraform output -json swagger_app_private_ip | jq '.' --raw-output > /var/lib/jenkins/env_vars/Swagger_Private_ip.txt                                
                terraform output -json elastic_cache_nodes | jq .[0].address --raw-output > /var/lib/jenkins/env_vars/Elasticache_Nodes.txt
                '''
                /*
